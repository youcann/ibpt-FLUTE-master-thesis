...
In this chapter a control system is designed and evaluated to stabilize \glspl{flute} RF system.
...
\begin{figure}[tb]
	...
	\caption[Identification sequence and response]{Section of the input sequence (blue) and the system response (orange); Note the inverse relation: A higher attenuation $A$ causes a lower cavity power $P_\text{cavity}$}
...
The manufacture's documentation \cite{pidTuner} does not disclose any internals of the PID Tuner nor state which optimization technique is used, but three tuning objectives are stated:
\begin{itemize}
...
\begin{figure}[tb]
	...
	\caption[Controller success time signal]{Time plots comparing between the control system on and off}
...
At the moment, the Faraday cup is mounted at the end of the low energy section. The capacity in the \gls{csa} is resetted at the start of every pulse and outputs a voltage proportional to the range setting and the measured charge.
